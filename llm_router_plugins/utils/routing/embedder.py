"""
Shared embedding-based router using a BiEncoder with a FAISS vector store.
...
The router is deliberately duck-typed on its configuration: it only needs the
``embedding_model``, ``chunk_size``, ``chunk_overlap``, ``top_k`` and
...
class EmbeddingRouterConfig:
    """
    Configuration contract for :class:`EmbeddingRouter`.

    A minimal, plugin-agnostic view of the semantic routing settings: the
    embedding model, the chunking parameters and the sequence of routing
    targets to index.

    Parameters
    ----------
    embedding_model : str
        The HuggingFace model identifier (or local path) used to compute
        embeddings.
    chunk_size : int
...
class EmbeddingRouter:
    ...
    ----------
    config : Any
        Routing configuration — must expose ``embedding_model``,
        ``chunk_size``, ``chunk_overlap``, ``top_k`` and ``routing_targets``
        (see :class:`EmbeddingRouterConfig` for a ready-made dataclass).
