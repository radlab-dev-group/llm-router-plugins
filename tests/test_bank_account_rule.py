...
class TestPolishIbanFormats:
    ...
    _TEST_CASES = [
        ("F1", "space after PL, groups of 4",
         "PL 73 1020 0010 5799 0000 0000 0000"),
        ("F2", "fully compact (default)",
         _PL_TEST_IBAN),
        ("F3", "dashed groups of 4",
         "PL-73-1020-0010-5799-0000-0000-0000"),
        ("F4", "space after PL, BBAN groups of 8",
         "PL 73 10200010 57990000 00000000"),
        ("F5", "compact country+check, rest spaced by 4",
         "PL73 1020 0010 5799 0000 0000 0000"),
        ("F6", "space after PL, bank as first 4-group",
...
class TestBoundaryAndContext:
    ...
    def test_multiple_iban_with_separator(self, rule: BankAccountRule):
        ...
        text = f"{iban1}\n{iban2}"
        result = _masked_text(rule, text)
        assert result.count("BANK_ACCOUNT") == 2, (
